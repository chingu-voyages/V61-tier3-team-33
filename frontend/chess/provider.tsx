...
import { ROOM_JOINED, GAME_STARTED, MOVE_MADE, MOVE_REJECTED, POSITION_ACCEPTED, POSITION_REJECTED, UNDO_APPLIED } from "@/socket/events"

export function ChessProvider({ children }: { children: React.ReactNode }) {
  const { send } = useSocketContext()

  const store = useMemo(
    () => new ChessStoreImpl(send),
    [send],
  )
  ...
  useSocketEvent(GAME_STARTED, (msg) => {
    ...
    store.setClock(
      msg.clock,
      msg.clock !== null ? performance.now() : null,
    )
  })

  useSocketEvent(MOVE_MADE, (msg) => {
    if (store.snapshot().pendingMove &&
  ...
  return (
    <ChessContext.Provider value={{ state, makeMove: store.makeMove, select: store.select, clearSelection: store.clearSelection, confirmPromotion: store.confirmPromotion, cancelPromotion: store.cancelPromotion }}>
      {children}
