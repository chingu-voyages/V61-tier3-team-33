...
export class Zobrist implements IHasher {
  ...
  private hashNormal(hash: bigint, moveHash: MoveHash): bigint {
    ...
    if (move.captured) {
      hash ^= this.piecePosition[this.tableOffset(this.pieceIndex(move.captured), move.to)]!;
    }
  ...
  private hashPromotion(hash: bigint, moveHash: MoveHash): bigint {
    ...
    hash ^= this.piecePosition[this.tableOffset(this.pieceIndex({ type: promoteTo, color: move.piece.color }), move.to)]!;
  ...
  private hashEnPassant(hash: bigint, moveHash: MoveHash): bigint {
    ...
    if (move.captured) {
      hash ^= this.piecePosition[this.tableOffset(this.pieceIndex(move.captured), Move.enPassantCapturedPosition(move))]!;
    }
  ...
  private hashCastlingRights(hash: bigint, moveHash: MoveHash): bigint {
    ...
    if (prevW.canCastleKingSide !== newW.canCastleKingSide) hash ^= this.castling[0]!;
    if (prevW.canCastleQueenSide !== newW.canCastleQueenSide) hash ^= this.castling[1]!;

    const prevB = moveHash.previousSides[1]!;
    const newB = moveHash.newSides[1]!;
    if (prevB.canCastleKingSide !== newB.canCastleKingSide) hash ^= this.castling[2]!;
    if (prevB.canCastleQueenSide !== newB.canCastleQueenSide) hash ^= this.castling[3]!;
  ...
  private hashEnPassantTarget(hash: bigint, moveHash: MoveHash): bigint {
    ...
    if (Move.isDoublePawnPush(moveHash.move)) {
      hash ^= this.enPassant[Position.file(Move.enPassantTarget(moveHash.move))]!;
    }
