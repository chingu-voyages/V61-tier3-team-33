...
export function getLegalMovesImpl(pieces: IPieces, moves: Move[], position: Position, ctx: TurnContext): Move[] {
  ...
  for (const move of moves) {
    ...
    // move leaves king in check → illegal, filter it out
    if (kingIsAttacked) {
      continue;
    }
...
export function getAllLegalMovesImpl(pieces: IPieces, moves: Move[], ctx: TurnContext): Move[] {
  ...
  // collect legal moves for every friendly piece
...
export function hasAnyLegalMovesImpl(pieces: IPieces, ctx: TurnContext): boolean {
  ...
  // search for any friendly piece that can make at least one legal move
  for (const [i, rawByte] of ctx.board.entries()) {
    const position = Position(i);
    const square = Square(rawByte);
    if (Square.isEmpty(square) || Square.isOccupiedBy(square, enemy)) {
