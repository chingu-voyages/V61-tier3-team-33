...
import { BLACK, HUMAN, HUMAN_VS_AI, HUMAN_VS_HUMAN, WHITE } from "../../types";
...
describe("Games", () => {
  ...
  describe("findWaiting", () => {
    ...
    it("self-heals by skipping and discarding stale queue entries", () => {
      ...
      (store as unknown as { games: Map<string, unknown> }).games.delete("room-1");

      expect(store.findWaiting(HUMAN_VS_HUMAN)).toBeNull();
      // The stale id should be gone now too, not just skipped.
      const queue = (store as unknown as { queue: Map<string, Set<string>> }).queue.get(`0:blitz`);
      expect(queue?.has("room-1")).toBe(false);
    });

    it("matches BLITZ when caller passes DEFAULT format (format fallback)", () => {
      const store = makeStore();
      store.create("room-1", HUMAN_VS_HUMAN); // created with default (blitz) clock

      // DEFAULT isn't a real format key — createClock() returns a BLITZ clock
      expect(store.findWaiting(HUMAN_VS_HUMAN, "DEFAULT" as never)).toBeNull();
    });
  });

  describe("commit", () => {
    it("stores the given game under the given id", () => {
      const store = makeStore();
      const original = store.create("room-1");
      const replacement = store.create("room-2"); // any Game instance works for this trivial impl

      store.commit("room-1", replacement);
  ...
  describe("drop", () => {
    ...
    it("works when game is not in any queue (already activated)", () => {
      const store = makeStore();
      const game = store.create("room-1", HUMAN_VS_HUMAN);
      game.join(WHITE, makeOccupant("p1"));
      game.join(BLACK, makeOccupant("p2")); // activates, removed from queue
