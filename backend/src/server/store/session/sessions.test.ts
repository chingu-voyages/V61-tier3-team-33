...
describe("Sessions", () => {
  ...
  describe("bind", () => {
    ...
    it("is a no-op for a socket with no session", () => {
      ...
      expect(() => store.bind(makeSocket(), { roomId: "room-1" })).not.toThrow();
    });
  });
  ...
  describe("reattachSocket", () => {
    it("returns null for an unknown playerId", () => {
      const store = makeStore();
      const ws = makeSocket();

      const result = store.reattachSocket(ws, "no-such-player");

      expect(result).toBeNull();
    });

    it("replaces the socket reference and clears disconnectedAt", () => {
      const store = makeStore();
      const oldWs = makeSocket();
      const session = store.open(oldWs, "player-1");
      store.drop(oldWs);
      ...
      const newWs = makeSocket();
      const result = store.reattachSocket(newWs, "player-1");

      expect(result).toBe(session);
      expect(result?.ws).toBe(newWs);
  ...
  describe("clearByPlayerId", () => {
    it("is a no-op for an unknown playerId", () => {
      const store = makeStore();

      expect(() => store.clearByPlayerId("no-such-player", "room-1")).not.toThrow();
    });

    it("is a no-op when roomId does not match expectedRoomId", () => {
      const store = makeStore();
      const ws = makeSocket();
      const session = store.open(ws, "player-1");
      store.bind(ws, { roomId: "room-1", color: WHITE, mode: HUMAN_VS_HUMAN });

      store.clearByPlayerId("player-1", "different-room");

      expect(session.roomId).toBe("room-1"); // unchanged
    });

    it("clears room fields when roomId matches", () => {
      const store = makeStore();
      const ws = makeSocket();
      const session = store.open(ws, "player-1");
      store.bind(ws, { roomId: "room-1", color: WHITE, mode: HUMAN_VS_HUMAN });

      store.clearByPlayerId("player-1", "room-1");
    ...
    it("is a no-op when session has already moved to a different room", () => {
      const store = makeStore();
      const ws = makeSocket();
      const session = store.open(ws, "player-1");
      store.bind(ws, { roomId: "room-1", color: WHITE, mode: HUMAN_VS_HUMAN });

      // simulated: player moved to a different room behind the scenes
      store.bind(ws, { roomId: "room-2" });

      store.clearByPlayerId("player-1", "room-1");

      // should NOT clear because actual roomId (room-2) != expected (room-1)
      expect(session.roomId).toBe("room-2");
    });
  });

  describe("drop edge cases", () => {
    it("is a no-op when called again on the same socket (already dropped)", () => {
      const store = makeStore();
      const ws = makeSocket();
      store.open(ws, "player-1");

      store.drop(ws); // first drop
      expect(() => store.drop(ws)).not.toThrow(); // second drop — no-op
    });
  });
});
